...
Status: v0.3.0 is live and verified at https://sns.kang88.io on 2026-09-17 through merged PR #4. `sns-kang88` uses Supabase Free ($0/month) in Seoul, project ref `ijflzexbgrzhrjcoljja`, under the confirmed `kang88xx's projects` organization. The private bucket, allowlisted operator, disabled public signup, and Vercel Production/Preview configuration are in place. Seven hosted API/RLS checks, six production app checks, and four real-provider photo browser checks passed. Security and performance advisors report no findings.
...
**Allow new users to sign up** is off in [Authentication settings](https://supabase.com/dashboard/project/ijflzexbgrzhrjcoljja/auth/providers). It was saved through the existing Windows Chrome session using native UI Automation; no browser debugging profile or new login was needed. API readback confirms `disable_signup: true`, and an attempted synthetic signup is rejected with HTTP 422 `signup_disabled`. The operator was created with confirmed email through the trusted Auth Admin API; no invitation email was sent. Its generated password is stored only in a private local file outside this repository.

Production deployment `dpl_HrWkiFcCRYaKjCER25zRg46EWB92` serves all ten public files byte-for-byte from the verified build. Production browser checks preserve seeded writing and verify local CRUD, calendar/filter/keyboard behavior, 360×500 controls, and real photo upload/reload/staged removal/publication cleanup. The temporary second QA account was signed out and deleted after the authorization tests; a database check confirmed one operator, one allowlist row, and zero remaining test photos. Temporary local administrative environment files were deleted. Retain the operator's private login file. See [deployment evidence](../DEPLOYMENT.md).
...
- Before merging, verify the configured preview and hosted access controls. After deployment, verify production HTTPS 200, all ten public files, existing calendar/CRUD/backup flows, 360px and short-height editor, and no app console/CSP errors before marking deployment verified.
